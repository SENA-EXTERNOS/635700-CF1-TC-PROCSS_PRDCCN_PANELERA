<template lang="pug">
.curso-main-container.pb-3
  BannerInterno(icono="fas fa-tasks" titulo="Actividad didáctica")
  .container.tarjeta.tarjeta--blanca.p-4.p-md-5.mb-5

    .titulo-segundo.color-acento-contenido.mb-5
      h2 Arrastrar y soltar

    .row
      .col-6.col-md-4.col-lg-3.mb-4.mb-md-0
        .tarjeta.h-100.d-flex.align-items-center.p-4
          figure
            img(src='@/assets/curso/temas/Actividad.svg')
      .col-6.col-md-8.col-lg-9
        p.mb-4 #[b Objetivo] La presente actividad que tiene como propósito validar la comprensión de los conceptos abordados. Para ello, debe relacionar cada uno de los términos con la definición correspondiente.
        .tarjeta.actividad.p-3
          .row.justify-content-around.align-items-center            
            .col-sm.mb-3.mb-sm-0
              p.fw-bold.mb-0 Arrastrar y soltar
            .col-auto
              a.boton.boton--b(:href="obtenerLink('/actividades/story.html')" target="_blank")
                span Realizar
                i.fas.fa-puzzle-piece
                
</template>

<script>
export default {
  name: 'Actividad',
}
</script>

<style lang="sass"></style>
